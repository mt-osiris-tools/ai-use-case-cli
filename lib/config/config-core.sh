...
# Creates ~/.config/ai-use-case-cli if it doesn't exist
#
# Usage:
#   ensure_config_dir
#
...
# Update configuration field (internal helper for complex updates)
# This is used internally by other config modules for adding new fields
# or handling jq-based updates
_update_config_field() {
    local key="$1"
    local value="$2"
    local value_type="${3:-string}"  # string, boolean, number
    ...
    # Require jq for complex updates
    if ! command -v jq &> /dev/null; then
        echo -e "${YELLOW}Warning: jq not available, using simple update${NC}" >&2
    ...
    case "$value_type" in
        boolean)
            jq --argjson val "$value" --arg key "$key" '.[$key] = $val' "$CONFIG_FILE" > "$temp_file"
            ;;
        number)
            jq --argjson val "$value" --arg key "$key" '.[$key] = $val' "$CONFIG_FILE" > "$temp_file"
            ;;
        *)
            jq --arg val "$value" --arg key "$key" '.[$key] = $val' "$CONFIG_FILE" > "$temp_file"
            ;;
    esac
